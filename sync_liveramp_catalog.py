...
class LiveRampCatalogSync:
    ...
    def store_segments(self, segments: List[Dict], append: bool = False):
        ...
        try:
            ...
            # Skip embedding generation during sync to avoid duplicates and rate limits
            # Embeddings are generated separately by the scheduled embeddings job
            print(f"  ℹ Skipping embedding generation during sync")
            print(f"  → Embeddings are generated by the scheduled job (runs daily at 3 AM UTC)")
        except Exception as e:
            # Rollback on error
            conn.rollback()
